...
router.post('/cadastrar', function (req, res, next) {
  ...
  banco.conectar().then(() => {
    ...
    peculiaridade = req.body.peculiaridade;
    senha = req.body.senha; // depois de .body, use o nome (name) do campo em seu formulário de login
    if (login == undefined || senha == undefined || nome == undefined || nascimento == undefined || peculiaridade == undefined ) {
	  // coloque a frase de erro que quiser aqui. Ela vai aparecer no formulário de cadastro
      throw new Error(`Dados de cadastro não chegaram completos: ${login} / ${senha} / ${nome}/ ${nascimento}/${peculiaridade}`);
    }
    ...
    banco.sql.query(`insert into cadastro (nome, email, nascimento, peculiaridade, senha) values 
    ('${nome}','${login}','${nascimento}','${peculiaridade}','${senha}')`).then(function() {
